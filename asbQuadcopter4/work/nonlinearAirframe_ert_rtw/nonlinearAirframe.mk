###########################################################################
## Makefile generated for component 'nonlinearAirframe'. 
## 
## Makefile     : nonlinearAirframe.mk
## Generated on : Mon May 31 13:20:12 2021
## Final product: $(RELATIVE_PATH_TO_ANCHOR)/nonlinearAirframe.so
...
PRODUCT_NAME              = nonlinearAirframe
MAKEFILE                  = nonlinearAirframe.mk
MATLAB_ROOT               = C:/PROGRA~1/MATLAB/R2020b
MATLAB_BIN                = C:/PROGRA~1/MATLAB/R2020b/bin
MATLAB_ARCH_BIN           = $(MATLAB_BIN)/win64
START_DIR                 = D:/SEMEST~1/GUIDAN~1/GNCPRO~1/LANDMA~1/ASBQUA~1/work
SOLVER                    = 
SOLVER_OBJ                = 
CLASSIC_INTERFACE         = 0
TGT_FCN_LIB               = None
...
PRODUCT = $(RELATIVE_PATH_TO_ANCHOR)/nonlinearAirframe.so
...
INCLUDES_BUILDINFO = -I$(START_DIR) -I$(START_DIR)/nonlinearAirframe_ert_rtw -I$(MATLAB_ROOT)/extern/include -I$(MATLAB_ROOT)/simulink/include -I$(MATLAB_ROOT)/rtw/c/src -I$(MATLAB_ROOT)/rtw/c/src/ext_mode/common -I$(MATLAB_ROOT)/rtw/c/ert -IC:/PROGRA~3/MATLAB/SUPPOR~1/R2020b/toolbox/target/SUPPOR~1/parrot/include
...
DEFINES_SKIPFORSIL = -DDELOS -DDELOS_EDU -DEXTMODE_DISABLETESTING -DEXTMODE_STATIC_SIZE=16384 -DEXTMODE_DISABLE_ARGS_PROCESSING=1 -DPARROT_LINUX_COMM -DSTACK_SIZE=64 -DRT -DSTOP_TIME=10 -DMAMBO=1
DEFINES_STANDARD = -DMODEL=nonlinearAirframe -DNUMST=2 -DNCSTATES=12 -DHAVESTDIO -DMODEL_HAS_DYNAMICALLY_LOADED_SFCNS=0
...
OBJS = jzdppihxbb.o nonlinearAirframe.o nonlinearAirframe_data.o rtGetInf.o rtGetNaN.o rt_nonfinite.o rsedu_control.o rsedu_of.o rsedu_vis.o ptimer.o controlCommand.o mw_extrathreads.o
...
%.o : $(START_DIR)/nonlinearAirframe_ert_rtw/%.c
	$(CC) $(CFLAGS) -o "$@" "$<"


%.o : $(START_DIR)/nonlinearAirframe_ert_rtw/%.cpp
	$(CPP) $(CPPFLAGS) -o "$@" "$<"
...
jzdppihxbb.o : $(START_DIR)/nonlinearAirframe_ert_rtw/jzdppihxbb.c
	$(CC) $(CFLAGS) -o "$@" "$<"


nonlinearAirframe.o : $(START_DIR)/nonlinearAirframe_ert_rtw/nonlinearAirframe.c
	$(CC) $(CFLAGS) -o "$@" "$<"


nonlinearAirframe_data.o : $(START_DIR)/nonlinearAirframe_ert_rtw/nonlinearAirframe_data.c
	$(CC) $(CFLAGS) -o "$@" "$<"


rtGetInf.o : $(START_DIR)/nonlinearAirframe_ert_rtw/rtGetInf.c
	$(CC) $(CFLAGS) -o "$@" "$<"


rtGetNaN.o : $(START_DIR)/nonlinearAirframe_ert_rtw/rtGetNaN.c
	$(CC) $(CFLAGS) -o "$@" "$<"


rt_nonfinite.o : $(START_DIR)/nonlinearAirframe_ert_rtw/rt_nonfinite.c
	$(CC) $(CFLAGS) -o "$@" "$<"
